...
export async function POST(request: NextRequest) {
  try {
    ...
    const currentNode = currentNodeId
      ? findNodeById(qaTree, currentNodeId)
      : null;
    ...
    try {
      ...
      function analyzeRequirement(text: string): {
        ...
        if (
          ...
          textLower.includes("intelligent") ||
          textLower.includes("smart")
        ) {
          tags.push("ai");
        }
      ...
      Object.values(updatedDocument.categories).forEach((category: any) => {
        if (Array.isArray(category.requirements)) {
          category.requirements = category.requirements.map(
            (req: string | any) => {
              ...
              if (existingReq) {
                ...
                return {
                  ...
                  tags: [
                    ...new Set([...(existingReq.tags || []), ...analysis.tags]),
                  ],
